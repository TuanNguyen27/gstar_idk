"""
Modal script for RL-based calibration training using vLLM for inference.
Uses REINFORCE (policy gradient) with anti-collapse rewards.
"""

import modal

app = modal.App("slm-rl-calibration-vllm")

# vLLM image for fast inference
vllm_image = modal.Image.debian_slim(python_version="3.10").pip_install(
    "vllm==0.6.3.post1",
    "torch==2.4.0",
    "transformers==4.45.2",  # Upgraded to meet vLLM requirement
)
...
training_image = (
    ...
    .pip_install(
        "numpy<2",
        "scipy",
        "torch==2.4.0",  # Upgraded for compatibility
        "transformers==4.45.2",  # Upgraded to meet vLLM requirement
        "peft==0.13.0",  # Latest version
        "datasets==2.15.0",
        "accelerate==0.34.0",
        "bitsandbytes==0.44.0",
        "tqdm",
...
def train_rl_calibration_model(
    ...
    num_epochs: int = 3,
    batch_size: int = 16,  # Larger since vLLM handles generation
    gradient_accumulation_steps: int = 4,
    lambda_sharp: float = 0.3,
    lambda_extreme: float = 0.2,
    generation_batch_size: int = 100,  # Generate episodes in batches
):
    """
    Train calibration model using RL (REINFORCE) with vLLM for generation.
    """
    import torch
    ...
    print(f"\n{'='*80}")
    print(f"RL-Based Calibration Training (REINFORCE + vLLM)")
    print(f"{'='*80}")
    ...
    print("\n" + "="*80)
    print("Starting RL Training (REINFORCE + vLLM)")
    print("="*80 + "\n")
    ...
    for epoch in range(num_epochs):
        ...
        # Training loop on generated episodes
...
def main(
    ...
    """
    Train RL-based calibration model with vLLM for generation.
    ...
    print(f"{'='*80}")
    print("RL CALIBRATION TRAINING (REINFORCE + vLLM)")
    print(f"{'='*80}")
    print(f"Model: {model}")
    ...
    # Train model
    output_path = f"{VOLUME_PATH}/rl_calibrated_models/{model_slug}_rl_vllm"
